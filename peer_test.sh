...
if [ -z "${TARGET_DEVICE}" ] ; then
  echo "Error: must specify a target device for peer-to-peer link experiments"
  exit 1
fi
...
latency_topic="peer"
throughput_topic="peer"
packet_loss_topic="peer"
downtime_topic="peer"
...
do
  ...
  if [ $? -eq 0 ] ; then
    ...
    echo $throughput Mbits/sec throughput
  else
    failed=1
    echo "iperf3 failed"
  fi
